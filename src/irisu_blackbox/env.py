...
class IrisuBlackBoxEnv(gym.Env[np.ndarray, int]):
    ...
    def reset(self, *, seed: int | None = None, options: dict[str, Any] | None = None):
        ...
        self.reward_shaper.reset(processed, raw)
    ...
    def step(self, action: int):
        ...
        reward, reward_terms = self.reward_shaper.step(processed, raw)
        self._step_count += 1

        backend_done = bool(getattr(self.backend, "game_over", False))
        terminated = backend_done or self.termination_detector.is_game_over(raw)
